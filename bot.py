...
# Хранилище ссылок (в памяти, для одного пользователя)
discussion_url = None


async def send_message(chat_id: int, text: str):
    """Отправляет сообщение в чат через прямой API-запрос."""
...
async def add_button_to_message(message_id: str, url: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.put(url_api, headers=headers, json=payload) as resp:
            if resp.status == 200:
...
async def start_command(event: MessageCreated):
    ...
    # Для ответа пользователю используем chat_id из recipient
