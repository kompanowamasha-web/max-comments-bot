...
async def add_button_to_message(message_id: str, discussion_url: str):
    keyboard = {
        ...
        "payload": {
            "buttons": [
                [{"type": "link", "text": "Прокомментировать", "url": discussion_url}]
            ]
        }
    }
...
@dp.message_created()
async def router(event: MessageCreated):
    ...
    if chat_type == ChatType.CHANNEL:
        ...
        if not discussion_url:
            logging.warning("⚠️ Нет ссылки на обсуждение. Напиши боту /start в личку.")
            return
    ...
    # Обработка ссылок при настройке
    if user_id not in user_settings:
    ...
    if waiting_for == "channel_url":
        ...
        await event.message.answer(
            "✅ Готово!\n\n"
            "Теперь опубликуй пост в канале — через 20-30 секунд появится кнопка."
        )
